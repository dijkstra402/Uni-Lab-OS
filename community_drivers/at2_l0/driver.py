...
Original: pcdsdevices/attenuator.py
...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.AT2L0")
...
from pcdsdevices.epics_motor import BeckhoffAxisNoOffset
...
from pcdsdevices.signal import InternalSignal, MultiDerivedSignal, MultiDerivedSignalRO
...
from pcdsdevices.valve import VCN, VVC
...
class AT2L0(FltMvInterface, PVPositionerPC, LightpathMixin):
    ...
    def _get_blade_error_attrs() -> Generator[str, None, None]:
        _unilab_logger.debug("[UNILAB] AT2L0._get_blade_error_attrs() called")
    ...
    def _check_errors(
        ...
        _unilab_logger.debug("[UNILAB] AT2L0._check_errors() called")
    ...
    def _check_errors_bitmask(
        ...
        _unilab_logger.debug("[UNILAB] AT2L0._check_errors_bitmask() called")
    ...
    def clear_errors(self):
        _unilab_logger.debug("[UNILAB] AT2L0.clear_errors() called")
    ...
    def _empty_get(self, mds: MultiDerivedSignal, items: SignalToValue) -> int:
        _unilab_logger.debug("[UNILAB] AT2L0._empty_get() called")
        return 0

    def _reset_errors(
        self, mds: MultiDerivedSignal, value: OphydDataType
    ) -> SignalToValue:
        _unilab_logger.debug("[UNILAB] AT2L0._reset_errors() called")
    ...
    def print_errors(self):
        _unilab_logger.debug("[UNILAB] AT2L0.print_errors() called")
        """prints the error summary """
        print(self.error_summary.get())

    @property
    def setpoint(self):
        _unilab_logger.debug("[UNILAB] AT2L0.setpoint() called")
        """(PVPositioner compat) - use desired transmission as setpoint."""
        return self.calculator.desired_transmission

    @property
    def readback(self):
        _unilab_logger.debug("[UNILAB] AT2L0.readback() called")
        """(PVPositioner compat) - use actual transmission as readback."""
        return self.calculator.actual_transmission

    @property
    def actuate(self):
        _unilab_logger.debug("[UNILAB] AT2L0.actuate() called")
        """(PVPositioner compat) - use apply_config as an actuation signal."""
        return self.calculator.apply_config

    def _setup_move(self, position):
        _unilab_logger.debug("[UNILAB] AT2L0._setup_move() called")
        """(PVPositioner compat) - calculate, then move."""
        # Do not call `calculator.calculate()` here to respect the current
        # calculator settings:
        self.calculator.desired_transmission.put(position)
    ...
    def get_lightpath_state(self, use_cache: bool = True) -> LightpathState:
        print("[UNILAB] AT2L0.__init__() called", flush=True)
        """
        Grab slightly different PV values for use in same inout calc fn
        The state is nested one device deeper than LightpathInOutCptMixin
        expects.
    ...
    def calc_lightpath_state(self, **lightpath_kwargs) -> LightpathState:
        _unilab_logger.debug("[UNILAB] AT2L0.calc_lightpath_state() called")
        # Repeat lightpath logic to extract num_in, num_out
        in_check = []
        out_check = []
        trans_check = []
    ...
    def format_status_info(self, status_info):
        _unilab_logger.debug("[UNILAB] AT2L0.format_status_info() called")
        """Override status info handler to render the attenuator."""
        calc_status = status_info.get('calculator', {})
        transmission = get_status_float(
            calc_status, 'actual_transmission', 'value',
