...
Original: src/ophyd_async/tango/core/_tango_transport.py
...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.AttributeProxy")
...
from ophyd_async.tango.core._converters import (
    TangoConverter,
...
from ophyd_async.tango.core._utils import DevStateEnum, get_device_trl_and_attr, try_to_cast_as_float
...
class AttributeProxy(TangoProxy):
    ...
    def __init__(self, *args, **kwargs):
        print("[UNILAB] AttributeProxy.__init__() called", flush=True)
    ...
    async def connect(self) -> None:
        _unilab_logger.debug("[UNILAB] AttributeProxy.connect() called")
        try:
    ...
    async def put(  # type: ignore
        ...
        _unilab_logger.debug("[UNILAB] AttributeProxy.get() called")
        # TODO: remove the timeout from this as it is handled at the signal level
        value = self._converter.write_value(value)
        try:

            async def _write():
                _unilab_logger.debug("[UNILAB] AttributeProxy._write() called")
    ...
    def has_subscription(self) -> bool:
        _unilab_logger.debug("[UNILAB] AttributeProxy.get_config() called")
        return bool(self._callback)

    @ensure_proper_executor
    async def _subscribe_to_event(self):
        _unilab_logger.debug("[UNILAB] AttributeProxy._subscribe_to_event() called")
        if not self._eid:
    ...
    def subscribe_callback(self, callback: Callback | None):
        _unilab_logger.debug("[UNILAB] AttributeProxy.subscribe_callback() called")
        ...
        if self.support_events:
            ...
            if self._callback is not None:
                ...
                async def _poll():
                    _unilab_logger.debug("[UNILAB] AttributeProxy._poll() called")
                    while True:
                        try:
                            await self.poll()
    ...
    def unsubscribe_callback(self):
        _unilab_logger.debug("[UNILAB] AttributeProxy.unsubscribe_callback() called")
        if self._eid:
            try:
    ...
    async def _event_processor(self, event):
        _unilab_logger.debug("[UNILAB] AttributeProxy._event_processor() called")
        if not event.err:
            reading = Reading(
    ...
    async def poll(self):
        _unilab_logger.debug("[UNILAB] AttributeProxy.poll() called")
    ...
    def set_polling(
        ...
        _unilab_logger.debug("[UNILAB] AttributeProxy.set_polling() called")
