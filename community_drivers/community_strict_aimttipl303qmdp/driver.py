...
class CommunityStrictAimttipl303qmdp(CommunityRepoLoader):
    DEFAULT_CONFIG = {'repo_root': '/Users/sml/work/Uni-Lab-OS/new/strict_candidates_repos/QCoDeS__Qcodes', 'source_file': 'src/qcodes/instrument_drivers/AimTTi/Aim_TTi_PL303QMD_P.py', 'class_name': 'AimTTiPL303QMDP', 'import_roots': ['src'], 'candidate_methods': ['get_idn', 'get_address', 'get_IP', 'get_netMask', 'get_netConfig', 'local_mode', 'is_interface_locked', 'lock_interface', 'unlock_interface', 'address', 'resource_manager', 'visa_handle', 'visabackend', 'visalib', 'set_address', 'device_clear', 'set_terminator', 'close', 'write_raw', 'ask_raw', 'snapshot_base', 'get_timeout', 'set_timeout', 'connect_message', 'close_all', 'record_instance', 'instances', 'remove_instance', 'find_instrument', 'exist', 'is_valid', 'label', 'add_parameter', 'remove_parameter', 'add_function', 'add_submodule', 'get_component', 'print_readable_snapshot', 'invalidate_cache', 'parent', 'ancestors', 'root_instrument', 'name_parts', 'full_name', 'name', 'short_name', 'set', 'get', 'call', 'validate_status', 'load_metadata', 'snapshot'], 'action_targets': {'get_timeout': '__qcodes_param_get__timeout', 'set_timeout': '__qcodes_param_set__timeout'}, 'metadata': {'repo': 'QCoDeS/Qcodes', 'repo_url': 'https://github.com/QCoDeS/Qcodes', 'source_url': 'https://github.com/QCoDeS/Qcodes/blob/main/src/qcodes/instrument_drivers/AimTTi/Aim_TTi_PL303QMD_P.py', 'confidence': 0.75, 'quality_score': 0.89, 'parse_status': 'ok', 'quality_status': 'usable', 'quality_reasons': [], 'action_targets': {'get_timeout': '__qcodes_param_get__timeout', 'set_timeout': '__qcodes_param_set__timeout'}}}
    ...
    def get_IP(self, **kwargs):
        return self.call('get_IP', kwargs=kwargs)

    def get_netMask(self, **kwargs):
        return self.call('get_netMask', kwargs=kwargs)

    def get_netConfig(self, **kwargs):
        return self.call('get_netConfig', kwargs=kwargs)

    def local_mode(self, **kwargs):
        return self.call('local_mode', kwargs=kwargs)

    def is_interface_locked(self, **kwargs):
        return self.call('is_interface_locked', kwargs=kwargs)

    def lock_interface(self, **kwargs):
        return self.call('lock_interface', kwargs=kwargs)

    def unlock_interface(self, **kwargs):
        return self.call('unlock_interface', kwargs=kwargs)
