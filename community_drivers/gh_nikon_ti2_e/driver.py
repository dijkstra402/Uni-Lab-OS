...
Original: slietar_nikon/nikon/microscope.py
"""
from typing import Dict, Any, Optional
import logging, sys as _sys
from pathlib import Path as _Path

_unilab_logger = logging.getLogger("unilab.driver.MicroscopeDevice")
_unilab_logger.setLevel(logging.DEBUG)
if not _unilab_logger.handlers:
    _h = logging.StreamHandler()
    _h.setFormatter(logging.Formatter("[%(asctime)s] %(message)s"))
    _unilab_logger.addHandler(_h)
...
class MicroscopeDevice:
  def __init__(self, device: usb.core.Device):
    ...
    self._call_executor = BatchExecutor(self._call_executor_run)
  ...
  async def _batch_call(self, call_type: int, payload: bytes):
    call_data = bytes([call_type]) + payload.rjust(4, b"\x00")
    await self._call_executor(call_data)

  async def _call_executor_run(self, data: list[bytes], /):
    async with self._lock:
      await self._request(b"\x01\x00\x21\xff\x00\x00" + bytes().join(data))
  ...
  async def set_x(self, value: int, /):
    ...
    await self._batch_call(0xa8, struct.pack(">i", value))
  ...
  async def set_y(self, value: int, /):
    ...
    await self._batch_call(0xac, struct.pack(">i", value))
  ...
  def list(cls, *, backend: Optional[usb.backend.IBackend] = None):
    ...
    Returns:
      Iterator[MicroscopeDevice]: An iterator which yields a `MicroscopeDevice` instance for each device.
    """

    for device in usb.core.find(backend=backend, find_all=True, idVendor=0x04b0, idProduct=0x7836): # type: ignore
      yield cls(device)
