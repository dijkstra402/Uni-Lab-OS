...
Original: nion/swift/model/HDF5Handler.py
...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.HDF5FileEntry")
...
class HDF5FileEntry:
    def __init__(self, path: pathlib.Path) -> None:
        print("[UNILAB] HDF5FileEntry.__init__() called", flush=True)
    ...
    def fp(self) -> _HDF5FilePointer:
        _unilab_logger.debug("[UNILAB] HDF5FileEntry.fp() called")
        with self.__lock:
            self.open()
            assert self.__fp
            return self.__fp
    ...
    def open(self) -> None:
        _unilab_logger.debug("[UNILAB] HDF5FileEntry.open() called")
        with self.__lock:
            if not self.__fp:
    ...
    def close(self) -> None:
        _unilab_logger.debug("[UNILAB] HDF5FileEntry.close() called")
        with self.__lock:
            if self.__fp:
                self.__fp.close()
            self.__fp = None
