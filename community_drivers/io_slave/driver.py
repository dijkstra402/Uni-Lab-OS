...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.IOSlave")
...
class IOSlave(object):
    def __init__(self, port='COM12'):
        print("[UNILAB] IOSlave.__init__() called", flush=True)
        self.ser_args = dict(port=port, baudrate=115200, rtscts=False, timeout=.1, writeTimeout=2)
        self.lock = threading.Lock()
        
    def SetDigital(self, chan, value):
        _unilab_logger.debug("[UNILAB] IOSlave.SetDigital() called")
        with self.lock:
    ...
    def SetAnalog(self, chan, value):
        _unilab_logger.debug("[UNILAB] IOSlave.SetAnalog() called")
        with self.lock:
            with serial.Serial(**self.ser_args) as ser:
                ser.write(b'SA%d %f\n' % (chan, value))
                res = ser.readline()
    ...
    def SetFlash(self, chan, value):
        _unilab_logger.debug("[UNILAB] IOSlave.SetFlash() called")
        with self.lock:
            with serial.Serial(**self.ser_args) as ser:
                ser.write(b'SF%d %d\n' % (chan, value))
                res = ser.readline()
    ...
    def GetAnalog(self, chan):
        _unilab_logger.debug("[UNILAB] IOSlave.GetAnalog() called")
        with self.lock:
            with serial.Serial(**self.ser_args) as ser:
                ser.write(b'QA%d\n' % chan)
    ...
    def GetTemperature(self, chan):
        _unilab_logger.debug("[UNILAB] IOSlave.GetTemperature() called")
        with self.lock:
            with serial.Serial(**self.ser_args) as ser:
                ser.write(b'QT%d\n' % chan)
...
class IOSlaveAlwaysOpen(object):
    ...
    def SetServo(self, chan, value):
        _unilab_logger.debug("[UNILAB] IOSlave.SetServo() called")
        with self.lock:
