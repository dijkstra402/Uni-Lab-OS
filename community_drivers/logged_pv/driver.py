...
Original: epicsapps/pvlogger/pvlogger.py
...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.LoggedPV")
...
from epicsapps.pvlogger.configfile import PVLoggerConfig
from epicsapps.instruments import InstrumentDB
from epicsapps.utils import normalize_pvname, normalize_path
...
class LoggedPV():
    ...
    def write(self, txt, flush=False):
        print("[UNILAB] LoggedPV.__init__() called", flush=True)
    ...
    def flush(self):
        _unilab_logger.debug("[UNILAB] LoggedPV.flush() called")
        self.next_flushtime = time() + 15.0
        with open(self.fpath, 'a', encoding='utf-8') as fh:
            fh.flush()
            self.needs_flush = False
    ...
    def set_mdel(self, mdel, mdelpv):
        _unilab_logger.debug("[UNILAB] LoggedPV.set_mdel() called")
        self.mdel = mdel
    ...
    def set_desc(self, desc, descpv):
        _unilab_logger.debug("[UNILAB] LoggedPV.set_desc() called")
        "set description"
        self.desc = desc
    ...
    def onConnect(self, pvname, conn, pv):
        _unilab_logger.debug("[UNILAB] LoggedPV.onConnect() called")
        ts = time()
        if conn:
    ...
    def onChanges(self, pvname, value, char_value='', timestamp=None, **kws):
        _unilab_logger.debug("[UNILAB] LoggedPV.onChanges() called")
        skip = False
    ...
    def save_current_value(self):
        _unilab_logger.debug("[UNILAB] LoggedPV.save_current_value() called")
        """
    ...
    def write_data(self):
        _unilab_logger.debug("[UNILAB] LoggedPV.write_data() called")
        if len(self.data) < 1:
            return
        buff = []
