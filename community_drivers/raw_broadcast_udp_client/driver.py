...
Original: lantz/drivers/legacy/rpc.py
...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.RawBroadcastUDPClient")
...
class RawBroadcastUDPClient(RawUDPClient):
    ...
    def __init__(self, bcastaddr, prog, vers, port):
        print("[UNILAB] RawBroadcastUDPClient.__init__() called", flush=True)
        RawUDPClient.__init__(self, bcastaddr, prog, vers, port)
        self.reply_handler = None
        self.timeout = 30
    
    def connect(self):
        _unilab_logger.debug("[UNILAB] RawBroadcastUDPClient.connect() called")
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_BROADCAST, 1)

    def set_reply_handler(self, reply_handler):
        _unilab_logger.debug("[UNILAB] RawBroadcastUDPClient.set_reply_handler() called")
        self.reply_handler = reply_handler

    def set_timeout(self, timeout):
        _unilab_logger.debug("[UNILAB] RawBroadcastUDPClient.set_timeout() called")
        self.timeout = timeout # Use None for infinite timeout

    def make_call(self, proc, args, pack_func, unpack_func):
        _unilab_logger.debug("[UNILAB] RawBroadcastUDPClient.make_call() called")
        if pack_func is None and args is not None:
            raise TypeError('non-null args with null pack_func')
        self.start_call(proc)
        if pack_func:
        ...
        while 1:
            r, w, x = [self.sock], [], []
            if select:
                _unilab_logger.debug("[UNILAB] RawBroadcastUDPClient.dummy() called")
