...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.UDPValuePub")
...
class UDPValuePub(object):
    ...
    def __init__(self, name, port = 37021):
        print("[UNILAB] UDPValuePub.__init__() called", flush=True)
        self.name = name
        self.port = port
    ...
    def run(self, values):
        _unilab_logger.debug("[UNILAB] UDPValuePub.run() called")
        packet = { "name": self.name, "val" : values }
        p = pickle.dumps(packet)
        z = zlib.compress(p)
    ...
    def shutdown(self):
        _unilab_logger.debug("[UNILAB] UDPValuePub.shutdown() called")
        self.sock.close()
