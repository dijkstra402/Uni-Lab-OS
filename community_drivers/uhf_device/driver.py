...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.UHFDevice")
...
class UHFDevice(ziShellDevice):
    def __init__(self, device="", host="", port=8004, interface="1GbE", api_level=5):
        print("[UNILAB] UHFDevice.__init__() called", flush=True)
        # Modules
        self.awgModule = None
    ...
    def connect_device(self, device, interface='1GbE'):
        _unilab_logger.debug("[UNILAB] UHFDevice.connect_device() called")
    ...
    def configure_awg_file(self, filename):
        _unilab_logger.debug("[UNILAB] UHFDevice.configure_awg_file() called")
        if not self.daq:
            raise(ziShellDAQError())
    ...
    def configure_awg_string(self, string):
        _unilab_logger.debug("[UNILAB] UHFDevice.configure_awg_string() called")
        if not self.daq:
            raise(ziShellDAQError())
    ...
    def read_scope(self, timeout=1.0, enable=True):
        _unilab_logger.debug("[UNILAB] UHFDevice.read_scope() called")
        if not self.daq:
            raise(ziShellDAQError())
    ...
    def restart(self):
        _unilab_logger.debug("[UNILAB] UHFDevice.restart() called")
        try:
