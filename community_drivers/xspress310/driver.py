...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.Xspress310")
...
class Xspress310(Device, ADFileMixin, Xspress3BaseMixin):
    ...
    def select_rois_to_save(self, roilist):
        print("[UNILAB] Xspress310.__init__() called", flush=True)
    ...
    def roi_calib_info(self):
        _unilab_logger.debug("[UNILAB] Xspress310.roi_calib_info() called")
        buff = ['[rois]']
        add = buff.append
        rois = self.get_rois()
    ...
    def restore_rois(self, roifile):
        _unilab_logger.debug("[UNILAB] Xspress310.restore_rois() called")
        """restore ROI setting from ROI.dat file"""
        cp =  ConfigParser()
        cp.read(roifile)
        rois = []
