...
class fibsem:
    ...
    def link_stage(self):
        ...
        print("No Linking on a Compustage available.")
        return()
    def unlink_stage(self):
        #microscope.specimen.stage.unlink()
        print("No Unlinking on a Compustage available.")
        return()
    ...
    def getStagePosition(self):
        ...
        try:
            stageposition=microscope.specimen.compustage.current_position
        except:
            stageposition=CompustagePosition(x=0,y=0,z=0,a=0)
        x=stageposition.x
        y=stageposition.y
        z=stageposition.z
        #r=stageposition.a
    ...
    def moveStageAbsolute(self, stageposition):
        ...
        ### Microscope Dependent Code ###
        stagepos=CompustagePosition(x=x,y=y,z=z,a=t)
        microscope.specimen.compustage.absolute_move(stagepos)
        return()
    def moveStageRelative(self,stageposition):
        ...
        ### Microscope Dependent Code ###
        stagepos=CompustagePosition(x=x,y=y,z=z,a=t)
        microscope.specimen.compustage.relative_move(stagepos)
        return("Stage Moved")
            ...
            if beam=='ION':
                ...
                while l.confidence < 0.98 and move_count < 3:
                    ...
                    if distance < 82.9e-06/3072/2:
                        ...
                        if rotation==0:
                            pos_corr = CompustagePosition(x=-x, y=-y)
                        ...
                        microscope.specimen.compustage.relative_move(pos_corr)
                        microscope.beams.ion_beam.beam_shift.value = Point(0,0)
            ...
            if beam=="ELECTRON":
                ...
                while l.confidence < 0.98 and move_count < 1:
                    ...
                    if distance > 1e-05:
                        ...
                        #pos_corr = StagePosition(coordinate_system='Specimen', x=x, y=y)
                        if num==0:
                            pos_corr = CompustagePosition(coordinate_system='Specimen', x=-x, y=-y)
                        if num==3.14:
                            pos_corr = CompustagePosition(coordinate_system='Specimen', x=x, y=y)
                        microscope.specimen.compustage.relative_move(pos_corr)
                        microscope.beams.electron_beam.beam_shift.value = Point(0,0)
        ...
        for i in range(0,step_num):
            if self.continuerun:
                if float(steps_current[i])==0:
                    #Testing Waffle workflow, rely on the apertures if trustApertures=True
    ...
    def run_milling_protocol(self, lamella_name, alignment_image, stagepos, pattern_ref_directory, protocol_filename,mode='fine'):
        ...
        self.moveStageAbsolute(stagepos)
        #FOR MACHINE LEARNING RUN
        #self.auto_focus(beam="ION")
        ref_img.save(patterns_output_directory[:-1] + '/initial_fine_alignment_img.tif')
        self.align(ref_img, 'ION')
