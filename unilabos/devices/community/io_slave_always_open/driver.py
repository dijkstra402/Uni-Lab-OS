...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.IOSlaveAlwaysOpen")
...
class IOSlaveAlwaysOpen(object):
    ...
    """
    def __init__(self, port='COM12'):
        print("[UNILAB] IOSlaveAlwaysOpen.__init__() called", flush=True)
        self.lock = threading.Lock()
        self.ser = serial.Serial(port=port, baudrate=115200, rtscts=False, timeout=.1, writeTimeout=2)

    def SetDigital(self, chan, value):
        _unilab_logger.debug("[UNILAB] IOSlaveAlwaysOpen.SetDigital() called")
        with self.lock:
            self.ser.write(b'SD%d %d\n' % (chan, value))
            self.ser.readline()

    def SetAnalog(self, chan, value):
        _unilab_logger.debug("[UNILAB] IOSlaveAlwaysOpen.SetAnalog() called")
        with self.lock:
            self.ser.write(b'SA%d %f\n' % (chan, value))
            self.ser.readline()

    def SetFlash(self, chan, value):
        _unilab_logger.debug("[UNILAB] IOSlaveAlwaysOpen.SetFlash() called")
        with self.lock:
            self.ser.write(b'SF%d %d\n' % (chan, value))
            self.ser.readline()

    def SetServo(self, chan, value):
        _unilab_logger.debug("[UNILAB] IOSlaveAlwaysOpen.SetServo() called")
        with self.lock:
            self.ser.write(b'SS%d %d\n' % (chan, value))
            self.ser.readline()

    def GetAnalog(self, chan):
        _unilab_logger.debug("[UNILAB] IOSlaveAlwaysOpen.GetAnalog() called")
        with self.lock:
    ...
    def GetTemperature(self, chan):
        _unilab_logger.debug("[UNILAB] IOSlaveAlwaysOpen.GetTemperature() called")
        with self.lock:
