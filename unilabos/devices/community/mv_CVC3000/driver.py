...
# !/usr/bin/env python
"""
"Vacuubrand_CVC_3000" -- API for Vacuubrand CVC3000 remote controllable vacuum controller
===================================
...
This provides a python class for Vacuubrand vacuum pumps with CVC 3000 vacuum controller.
The command implementation is based on the english manual: English manual version: 999222 / 11/03/2016 Pages 40 - 45
...
class CVC3000(SerialDevice):
    ...
    def __init__(self, port=None, device_name=None, connect_on_instantiation=True, soft_fail_for_testing=False, address=None, mode=None):
        """
        Initializer of the CVC3000 class
        ...
        # Flag to determine if currently running or not
        self.running = False
    ...
    def initialise(self, controller_version="CVC 3000"):
        ...
        Args:
            controller_version (str): desired controller version ("CVC 2000" or "CVC 3000")
        """
    ...
    def set_mode(self, mode):
        ...
        Args:
            mode (str): the mode to be set. Must be a valid key in the MODES_3 dict.

        Returns:
            call back to __send_message with an order to set the mode
        """
        try:
    ...
    def start(self):
        ...
        Returns:
            True if pump responds properly, else a ValueError is raised.
        """
        if self.running:
            self.logger.debug("Device already operating")
            return True
    ...
    def stop(self, stop_mode=None):
        ...
        if not self.running:
            self.logger.debug("Device has already been stopped")
            return True
    ...
    def vent(self, vent_status=1):
        ...
        Args:
            vent_status (int): 0: valve closed; 1: valve open; 2: vent to atmospheric pressure
