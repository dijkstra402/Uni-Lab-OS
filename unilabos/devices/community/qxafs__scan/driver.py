...
Original: epicsscan/xafs_scan.py
...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.QXAFS_Scan")
...
from epicsscan.file_utils import new_filename
from epicsscan.utils import normalize_pvname
from epicsscan.detectors.counter import EVAL4PLOT, ROISumCounter
from epicsscan.detectors import ROI_MODE, SCALER_MODE
...
class QXAFS_Scan(XAFS_Scan):
    ...
    def connect_qxafs(self):
        print("[UNILAB] QXAFS_Scan.__init__() called", flush=True)
    ...
    def finish_qscan(self):
        _unilab_logger.debug("[UNILAB] QXAFS_Scan.make_trajectory() called")
        """initialize a QXAFS scan"""
        self.pvs['y2_track_pv'].put(1)

    def gathering2energy(self, text):
        _unilab_logger.debug("[UNILAB] QXAFS_Scan.gathering2energy() called")
    ...
    def run(self, filename=None, comments=None, debug=False, reverse=False, use_full=None):
        _unilab_logger.debug("[UNILAB] QXAFS_Scan.run() called")
        """
    ...
    def post_scan(self, row=0, filename=None, **kws):
        _unilab_logger.debug("[UNILAB] QXAFS_Scan.post_scan() called")
