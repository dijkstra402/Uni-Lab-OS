...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.SiTcp")
...
class SiTcp(SiTransferLayer):
    ...
    def __init__(self, conf):
        print("[UNILAB] SiTcp.__init__() called", flush=True)
    ...
    def reset(self):
        _unilab_logger.debug("[UNILAB] SiTcp.reset() called")
        with self._tcp_lock:
            self._tcp_read_buff = array('B')

    def reset_fifo(self):
        _unilab_logger.debug("[UNILAB] SiTcp.reset_fifo() called")
        with self._tcp_lock:
    ...
    def init(self):
        _unilab_logger.debug("[UNILAB] SiTcp.init() called")
    ...
    def _write_single(self, addr, data):
        _unilab_logger.debug("[UNILAB] SiTcp._write_single() called")
        retry_write_cnt = 0
        while True:
            if self.RBCP_ID >= 255:
                self.RBCP_ID = 0
    ...
    def write(self, addr, data):
        _unilab_logger.debug("[UNILAB] SiTcp.write() called")
    ...
    def _read_single(self, addr, size):
        _unilab_logger.debug("[UNILAB] SiTcp._read_single() called")
        retry_write_cnt = 0
        while True:
            if self.RBCP_ID >= 255:
                self.RBCP_ID = 0
    ...
    def read(self, addr, size):
        _unilab_logger.debug("[UNILAB] SiTcp.read() called")
        if addr < self.BASE_DATA_TCP:
            ret = array('B')
            with self._udp_lock:
...
#                 logger.warning("SiTcp:read - Invalid address %s" % hex(addr))
    ...
    def _tcp_readout(self):
        _unilab_logger.debug("[UNILAB] SiTcp._tcp_readout() called")
        time_read = time()
    ...
    def _get_tcp_data_size(self):
        _unilab_logger.debug("[UNILAB] SiTcp._get_tcp_data_size() called")
        with self._tcp_lock:
            size = len(self._tcp_read_buff)
        return size

    def _get_tcp_data(self, size):
        _unilab_logger.debug("[UNILAB] SiTcp._get_tcp_data() called")
        with self._tcp_lock:
    ...
    def _send_tcp_data(self, data):
        _unilab_logger.debug("[UNILAB] SiTcp._send_tcp_data() called")
        total_sent = 0
        while not self._stop:
    ...
    def _reset_tcp_to_bus(self):
        _unilab_logger.debug("[UNILAB] SiTcp._reset_tcp_to_bus() called")
        self._send_tcp_data(array('B', [255] * 65535))
        self._send_tcp_data(array('B', [0] * 6))

    def close(self):
        _unilab_logger.debug("[UNILAB] SiTcp.close() called")
