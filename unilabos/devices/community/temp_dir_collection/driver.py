...
# Unilab 驱动调试日志
_unilab_logger = logging.getLogger("unilab.driver.TempDirCollection")
...
class TempDirCollection(DirCollection):
    ...
    def __init__(self, fsdir, virdir, parent=None):
        print("[UNILAB] TempDirCollection.__init__() called", flush=True)
        self.fsname = fsdir
        self.name = virdir
        if self.fsname[-1] != os.sep:
            if self.fsname[-1] == '/': # fixup win/dos/mac separators
    ...
    def getProperties(self):
        _unilab_logger.debug("[UNILAB] TempDirCollection.getProperties() called")
        """Return dictionary with WebDAV properties. Values shold be
        formatted according to the WebDAV specs."""
    ...
    def getMembers(self):
        _unilab_logger.debug("[UNILAB] TempDirCollection.getMembers() called")
        r = []
    ...
    def findMember(self, name):
        _unilab_logger.debug("[UNILAB] TempDirCollection.findMember() called")
        if (self.fsname + name).rstrip('/') in _temporary_directories:
            return TempDirCollection(self.fsname + name, self.virname + name, self)
    
    def recvMember(self, rfile, name, size, req):
        _unilab_logger.debug("[UNILAB] TempDirCollection.recvMember() called")
