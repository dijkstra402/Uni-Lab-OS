...
Original: src/tomato_jumo/__init__.py
...
# Vendored repo packages: tomato_jumo
import sys as _sys
...
# 0x0025 - controller logic (1 = 0x8001, 2 = 0x8002 ....)
...
class Device(ModelDevice):
    ...
    def temperature(self) -> pint.Quantity:
        _unilab_logger.debug("[UNILAB] Device.temperature() called")
    ...
    def setpoint(self) -> pint.Quantity:
        _unilab_logger.debug("[UNILAB] Device.setpoint() called")
        val = self.instrument.read_float(
    ...
    def duty_cycle(self) -> pint.Quantity:
        _unilab_logger.debug("[UNILAB] Device.duty_cycle() called")
        val = self.instrument.read_float(
    ...
    def heater_on(self) -> bool:
        _unilab_logger.debug("[UNILAB] Device.heater_on() called")
        val = self.instrument.read_register(
            registeraddress=PARAM_MAP["heater_on"],
        )
        self.last_action = time.perf_counter()
        if val == 0x8001:
            ret = True
    ...
    def attrs(self, **kwargs) -> dict[str, Attr]:
        _unilab_logger.debug("[UNILAB] Device.attrs() called")
    ...
    def set_attr(self, attr: str, val: pint.Quantity, **kwargs: dict) -> pint.Quantity:
        _unilab_logger.debug("[UNILAB] Device.set_attr() called")
        if attr in {"ramp_rate", "ramp_target"}:
            setattr(self, attr, val)
        elif attr in {"heater_on"}:
            register_nr = PARAM_MAP[attr]
            self.instrument.write_register(
                registeraddress=register_nr, value=0x8001 if val is True else 0x8010
            )
            self.last_action = time.perf_counter()
        else:
            register_nr = PARAM_MAP[attr]
    ...
    def get_attr(self, attr: str, **kwargs: dict) -> pint.Quantity:
        _unilab_logger.debug("[UNILAB] Device.get_attr() called")
        if attr not in self.attrs():
            raise AttributeError(f"unknown attr: {attr!r}")
        return getattr(self, attr)

    def capabilities(self, **kwargs) -> set:
        _unilab_logger.debug("[UNILAB] Device.capabilities() called")
        caps = {"constant_temperature", "temperature_ramp"}
        return caps

    def do_measure(self, **kwargs) -> None:
        _unilab_logger.debug("[UNILAB] Device.do_measure() called")
        setp = self.setpoint
    ...
    def prepare_task(self, task: Task, **kwargs: dict) -> None:
        _unilab_logger.debug("[UNILAB] Device.prepare_task() called")
    ...
    def stop_task(self, **kwargs: dict) -> None:
        _unilab_logger.debug("[UNILAB] Device.stop_task() called")
    ...
    def reset(self, **kwargs) -> None:
        ...
        if self.ramp_task.do_run is True:
            self.ramp_task.do_run = False
            self.ramp_task.join()
        self.set_attr(attr="heater_on", val=False)
        self.set_attr(attr="setpoint", val=200001)

    def _temperature_ramp(self) -> None:
        _unilab_logger.debug("[UNILAB] Device._temperature_ramp() called")
